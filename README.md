<div align=center><h1> YOUTUBE_PROJECT </h1></div>

<div align=center>
Spring Boot를 직접 사용하며 공부하기 위해, 이를 사용해 제가 생각하는 YOUTUBE의 일부 핵심 기능을 구현했습니다.

</div>

## 신경 쓴 부분
1. 커스텀 에러 코드 생성
   - Http Status Code만으로는 정확한 에러 원인을 알 수 없을 때, 구체적인 에러를 안내하고 프론트 개발자와 원활한 개발 소통을 하고자 별도로 에러 코드를 처리해 적절한 오류 메시지를 사용하였습니다.  (com.example.demo.config.BaseResponseStatus)
   
     <br>
   
2. JWT
   - HTTP의 state-less 특성에서 생기는 첫 번째 단점인 '데이터 노출로 인한 보안 문제'를 해결하기 위해, JWT로 암호화하고자 JWT를 사용했습니다.
   - HTTP의 state-less 특성에서 생기는 두 번째 단점인 '많은 양의 데이터를 반복적으로 전송함으로써 발생하는 네트워크 성능저하 문제'를 해결하기 위해, JWT로 데이터 압축하고자 JWT를 사용했습니다.
   - 다중 서버 환경을 고려했을 때, '세션' 방식 대비 각각 로드 밸런싱 된 서버에 토큰 검증 로직이 있다면 인가 과정에 문제가 발생하지 않으므로 JWT를 사용했습니다.
   
    <br>

3. REST API URI
   - REST API URI에서의 URI 작성 기본 규칙을 준수했습니다.
   - REST API의 구성요소인 '자원', '행위', '표현'을 이해하고, 적용했습니다. 
   - URI에서 리소스 간의 관계를 표현했습니다.
   
     <br>

4. 비밀번호 암호화
   - 데이터베이스에 저장되는 비밀번호도 보안을 위해, 복호화가 불가능한 단방향 암호화 알고리즘 SHA-256을 사용해 암호화를 진행했습니다.
   
    <br>

5. MVC 패턴
   - Spring Boot 에서 적용되는 MVC 패턴 및 이에 따른 Controller, Service, DAO 의 역할(Spring MVC 패턴 패키지 구조)을 이해하고, 해당 구조를 적용했습니다.

    <br>
   
6. Spring Boot에서 Http Method 매핑
   - 특히, Get 요청은 Request Body 를 허용하지 않으며, uri에 요청 정보를 담아야하기 때문에 파라미터가 노출돼서 보안에 취약할 수 있는 특징을 이해하고 구현했습니다.
...
<img src="https://img.shields.io/badge/jdbc-6DB33F?style=plastic&logo=jdbc&logoColor=white"> 
